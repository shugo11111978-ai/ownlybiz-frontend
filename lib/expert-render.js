...
function renderExpertFirstPaint(result, page, pages) {
  ...
  const background = /^#[0-9a-f]{3,8}$/i.test(wc.design_tokens && wc.design_tokens.background || '') ? wc.design_tokens.background : wc.site_mode === 'dark' ? '#0B0908' : '#F7F1E8';
  const foreground = wc.site_mode === 'dark' ? '#F6F1E9' : '#252126';
  return `<style id="ob-expert-first-paint-css">
html.ob-public-first-paint #view-4{display:none!important}
html.ob-public-first-paint body{opacity:1!important}
#ob-public-first-paint-shell{position:relative;z-index:5;min-height:100vh;background:#faf8f5;color:#252126;font:16px/1.65 system-ui,sans-serif}
#ob-public-first-paint-shell nav{display:flex;flex-wrap:wrap;gap:14px;align-items:center;padding:22px max(20px,calc((100vw - 1000px)/2));border-bottom:1px solid #ded8e3}
#ob-public-first-paint-shell a{color:${accent};text-underline-offset:4px}
#ob-public-first-paint-shell main{max-width:860px;padding:44px 22px 72px;margin:auto}
#ob-public-first-paint-shell h1{font-size:clamp(30px,5vw,48px);line-height:1.2}
#ob-public-first-paint-shell h2{font-size:24px;line-height:1.3}
#ob-public-first-paint-shell section{margin:26px 0}
#ob-public-first-paint-shell img{display:block;max-width:100%;max-height:440px;object-fit:cover;border-radius:18px;margin:20px 0}
#ob-public-first-paint-shell .ob-ssr-book{display:inline-block;padding:12px 24px;border-radius:8px;color:white;background:${accent};text-decoration:none}
#ob-public-first-paint-shell footer{padding:20px;text-align:center}
#ob-public-first-paint-shell .ob-first-paint-loading{display:none}
html.ob-public-hydrating body{opacity:1!important;background:${background}!important}
html.ob-public-hydrating #views-container{visibility:hidden}
html.ob-public-hydrating #ob-public-first-paint-shell{height:100vh;overflow:hidden;background:${background};color:${foreground}}
html.ob-public-hydrating #ob-public-first-paint-shell> :not(.ob-first-paint-loading){visibility:hidden}
html.ob-public-hydrating #ob-public-first-paint-shell .ob-first-paint-loading{display:block;position:absolute;top:40vh;left:20px;right:20px;text-align:center}
</style><div id="ob-public-first-paint-shell" data-slug="${escape(result.slug)}" data-page="${escape(key)}"><div class="ob-first-paint-loading" role="status" aria-live="polite">Loading ${escape(name)}…</div><nav aria-label="Website"><strong>${escape(name)}</strong>${pages.filter(item => item.showInNav !== false).map(item => `<a href="${escape(pagePath(item))}"${pageKey(item) === key ? ' aria-current="page"' : ''}>${escape(item.label || item.nav_label || ({home:'Home',about:'About',services:'Services',reviews:'Reviews',book:'Book a session',contact:'Contact'}[pageKey(item)]) || item.title || pageKey(item))}</a>`).join('')}</nav><main><h1>${escape(heading)}</h1>${body}${bookHref && key !== 'book' ? `<p><a class="ob-ssr-book" href="${escape(bookHref)}">${escape(text(wc.hero_cta) || 'Book a session')}</a></p>` : ''}</main><footer>© ${new Date().getFullYear()} ${escape(text(expert.footer_text) || name)}</footer></div>`;
}
