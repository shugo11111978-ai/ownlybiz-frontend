...
async function route(request,response){
  ...
  if(path===base&&request.method==='GET'){send(response,200,{success:true,conversations:[...store.conversations.values()].reverse().slice(0,20).map(({turns,...row})=>row)});return;}
  if(path===base&&request.method==='POST'){const now=Math.floor(Date.now()/1000),conversation={id:randomUUID(),mode:input.mode,revision:1,created_at:now,updated_at:now,turns:[]};store.conversations.set(conversation.id,conversation);send(response,200,{success:true,conversation:dto(store,conversation)});return;}
  ...
  if(match[2]==='messages'&&request.method==='POST'){
    ...
    const delay=store.delay;store.delay=0;await new Promise(resolve=>setTimeout(resolve,delay||450));
    ...
    if(conversation.mode==='teach')turn.proposed_lessons=['Acknowledge the client’s latest concern naturally and retain relevant details shared earlier in the current conversation.'];conversation.revision++;conversation.updated_at=Math.floor(Date.now()/1000);send(response,200,{success:true,conversation:dto(store,conversation)});return;
  }
