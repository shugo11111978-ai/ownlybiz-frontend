...
assert.doesNotMatch(markup,/ob-ra-(?:gate-auto-send|auto-send-consent|auto-send-version|auto-send-status)/,'Human Reply Assistant displays no Automatic AI Chat consent state or control');
assert.match(markup,/Enable Human Reply Assistant[\s\S]*?never authorizes automatic sending by itself/,'the human drafting switch explicitly denies automatic-send authority');
...
assert.match(markup,/id="ob-ra-expert-guidance"[^>]+maxlength="6000"/,'existing expert-written guidance uses the governed 6,000-character bound');
...
assert.match(automaticTraining,/Real client transcripts are not copied into this trainer[\s\S]*?never transcript content/,'Automatic Chat learning provenance explicitly excludes raw transcripts');
...
assert.match(source,/HUMAN_GUIDANCE='\/api\/ai\/reply-assistant\/guidance'/,'Human Reply Assistant guidance uses its dedicated authority endpoint');
assert.match(source,/expected_revision:state\.controls\.revision/,'controls use exact revision CAS');
assert.doesNotMatch(humanControlMutation,/autoSend|auto_send|auto-send/,'Human Reply Assistant control drafts and PUT bodies never read or mutate Automatic AI Chat consent');
...
const automaticConsentSandbox={window:null,Number};automaticConsentSandbox.window=automaticConsentSandbox;
vm.createContext(automaticConsentSandbox);
new vm.Script(`${automaticConsentParser}\nwindow.__parseAutomaticConsent=automaticChatConsentState;`,{filename:'automatic-ai-chat-consent-parser.js'}).runInContext(automaticConsentSandbox);
const currentAutomaticConsent=automaticConsentSandbox.__parseAutomaticConsent({consent:{accepted:true,revision:7,document_version:'auto-v2',current_document_version:'auto-v2'}});
assert.equal(currentAutomaticConsent.accepted,true);assert.equal(currentAutomaticConsent.revision,7);assert.equal(currentAutomaticConsent.currentDocumentVersion,'auto-v2');
...
function createHarness({role='expert',support=false,mini=false,fetchImpl=null,visible=true}={}){
  ...
  for(const id of ['ob-ra-gate-entitlement','ob-ra-gate-expert','ob-ra-gate-training','ob-ra-gate-training-consent'])document.add('div',id,manager);
  for(const id of ['ob-ra-enabled','ob-ra-training-consent']){const input=document.add('input',id,manager);input.disabled=true;}
...
assert.deepEqual(primary.calls.map((call)=>new URL(call.url).pathname),['/api/ai/reply-assistant/controls','/api/ai/reply-assistant/sources','/api/ai/reply-assistant/guidance']);
...
assert.equal(primary.document.getElementById('ob-ra-enabled').checked,false);assert.equal(primary.document.getElementById('ob-ra-training-consent').checked,false);
assert.equal(primary.document.getElementById('ob-ra-training-version').textContent,'training-v1');
assert.equal(primary.document.getElementById('ob-ra-auto-send-consent'),null);assert.equal(primary.document.getElementById('ob-ra-auto-send-status'),null,'the Human Reply Assistant runtime creates no automatic-send UI');
...
primary.hooks.renderControls(allReady);assert.equal(primary.document.getElementById('ob-ra-auto-send-status'),null,'rendering a compatibility envelope never projects automatic consent into Human Reply Assistant');
...
const oversized=createHarness({fetchImpl:(url)=>url.endsWith('/controls')?response(controls()):url.endsWith('/sources')?response({success:true,sources:Array.from({length:401},(_,index)=>sourceRow(`bulk-${index}`))}):response({success:true,available:true,guidance:'',revision:0})});
...
primary.hooks.renderControls(primary.hooks.parseControls(controls({controls:{auto_send_consent:true,auto_send_consent_version:'auto-v1',auto_send_consented_at:1700001001}})));
const enabled=primary.document.getElementById('ob-ra-enabled'),training=primary.document.getElementById('ob-ra-training-consent');
enabled.checked=true;enabled.dispatchEvent({type:'change'});training.checked=true;training.dispatchEvent({type:'change'});primary.retention.value='45';primary.retention.dispatchEvent({type:'change'});
...
assert.deepEqual(JSON.parse(primary.calls.at(-1).options.body),{expected_revision:1,reply_assistant_enabled:true,training_consent:true,training_consent_version:'training-v1',retention_days:45},'Human Reply Assistant PUT includes only human drafting changes and exact CAS, never Automatic AI Chat consent');
assert.equal(primary.hooks.state.controls.revision,2);assert.equal(primary.hooks.state.effective.trainingAvailable,true);

const guidanceWait=deferred();primary.guidance.value='Submitted guidance';primary.guidance.dispatchEvent({type:'input'});primary.setHandler((url,options)=>{assert(url.endsWith('/api/ai/reply-assistant/guidance'));assert.equal(options.method,'PUT');assert.deepEqual(JSON.parse(options.body),{guidance:'Submitted guidance',expected_revision:1});return guidanceWait.promise;});
const guidanceSave=primary.hooks.saveGuidance();primary.guidance.value='Newer guidance stays local';primary.guidance.dispatchEvent({type:'input'});guidanceWait.resolve(response({success:true,available:true,guidance:'Submitted guidance',revision:2}));
...
rotated.setHandler((url,options)=>{
  ...
  if(token==='Bearer token-a'&&url.endsWith('/guidance')&&options.method==='PUT'){assert.deepEqual(JSON.parse(options.body),{guidance:'Guidance draft survives token refresh',expected_revision:1});return guidanceOld.promise;}
  assert.equal(token,'Bearer token-refresh');
  if(url.endsWith('/controls'))return response(controls());if(url.endsWith('/sources'))return response({success:true,sources:[sourceRow('pending-one','pending_review',{revision:8,content:'Server revision after refresh'})]});if(url.endsWith('/guidance'))return response({success:true,available:true,guidance:'Server guidance after refresh',revision:8});throw new Error(`unexpected rotation request ${url}`);
});
...
const unsafe=createHarness({fetchImpl:(url)=>url.endsWith('/controls')?response({success:false,error:'RAW_DATABASE_PROVIDER_DETECTOR_SECRET',code:'unexpected_internal_code'},500):url.endsWith('/sources')?response({success:true,sources:[]}):response({success:true,available:true,guidance:'',revision:0})});await unsafe.start();
