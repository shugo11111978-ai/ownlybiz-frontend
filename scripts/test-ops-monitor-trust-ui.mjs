...
const sandbox={
  console,window:null,document,localStorage,sessionStorage,location:{hostname:'ownlybiz.example.test',href:'https://ownlybiz.example.test/admin'},
...
assert.match(authorities,/Suppressed by recipient safety<\/td><td>37[\s\S]*not dead or permanently failed jobs/i,'recipient-safety suppressions render as a distinct informational outcome');
assert.doesNotMatch(authorities,/Dead 37/,'suppressed jobs are never presented as dead jobs');
...
assert.match(authorities,/Stripe runtime[\s\S]*?LIVE/,'active production Stripe live mode is rendered');
assert.match(authorities,/Active configuration ready[\s\S]*?yes[\s\S]*?Configured mode[\s\S]*?LIVE/,'the exact active Stripe live configuration readiness and coherent configured mode are visible');
...
assert.match(classifiedAuthorities,/Runtime classification[\s\S]*?production[\s\S]*?Raw provider environment[\s\S]*?production/,'normalized production classification and the raw provider label remain separately visible');
...
const paymentDisputeStates=['observed','open','deferred_cross_border','reversed','compensation_pending','compensated','lost_recovered','closed_no_clawback','no_destination_transfer','recovery_satisfied_elsewhere','manual_review'];
...
const expertSubscriptionStatuses=['active','trialing','payment_action_required','past_due','unpaid','incomplete','incomplete_expired','canceled','paused','pending_approval','signup_draft','checkout_pending','checkout_payment_pending','checkout_payment_failed','checkout_expired','change_payment_pending'];
const aiCreditCheckoutStatuses=['creating','pending','processing','paid','failed','expired'];
const groupCapacityStatuses=['draft','creating','sent','processing','paid','failed','expired','cancelled'];
const groupTicketStatuses=['creating','pending','processing','paid','failed','expired'];
...
const healthyOverview=withOverviewProvenance({
  ...
  media_sfu:healthyMediaSfu,security:healthySecurity,reply_assistant:healthyReplyAssistant,payment_runtime:{available:true,status:'ok',active_stripe_mode:'live',active_configuration_ready:true,webhook_configuration_ready:true,configured_mode:'live',last_error_code:null},
});
...
const replyOverviewGeneratedMs=Date.parse(healthyOverview.generated_at),replyOverviewGeneratedSec=Math.floor(replyOverviewGeneratedMs/1000);
const initializingReplyAssistant={...healthyReplyAssistant,running:true,last_run_at:replyOverviewGeneratedSec,last_run_age_sec:0,started_at:replyOverviewGeneratedSec-60,worker_age_sec:60,current_run_started_at:null,current_run_age_sec:null};
assert.equal(hooks.validOpsReplyAssistant(initializingReplyAssistant,replyOverviewGeneratedMs,healthyThresholds),true,'a started running worker may report the narrow run-start transition before its current-run DB timestamp is visible');
const initializingReplyOverview=withOverviewProvenance({...healthyOverview,reply_assistant:initializingReplyAssistant,summary:{...healthyOverview.summary,reply_assistant_status:'ok'}});
assert.equal(hooks.validOpsOverview(initializingReplyOverview),true,'the full overview accepts a fresh Reply Assistant run-start transition without inventing a warning');
assert.equal(hooks.validOpsReplyAssistant({...initializingReplyAssistant,started:false,started_at:null,worker_age_sec:null},replyOverviewGeneratedMs,healthyThresholds),false,'a running worker can never validate unless it is also started');
assert.equal(hooks.validOpsReplyAssistant({...initializingReplyAssistant,current_run_age_sec:0},replyOverviewGeneratedMs,healthyThresholds),false,'a null current-run timestamp requires a null current-run age');
const activeReplyAssistant={...initializingReplyAssistant,current_run_started_at:replyOverviewGeneratedSec-5,current_run_age_sec:5};
assert.equal(hooks.validOpsReplyAssistant(activeReplyAssistant,replyOverviewGeneratedMs,healthyThresholds),true,'a normal active run retains exact timestamp/derived-age coherence');
assert.equal(hooks.validOpsReplyAssistant({...activeReplyAssistant,current_run_age_sec:100},replyOverviewGeneratedMs,healthyThresholds),false,'a non-null current-run timestamp with an incoherent derived age fails closed');
const staleInitializingReplyAssistant={...initializingReplyAssistant,status:'critical',reason_codes:['worker_stalled'],last_run_at:replyOverviewGeneratedSec-211,last_run_age_sec:211,started_at:replyOverviewGeneratedSec-300,worker_age_sec:300};
assert.equal(hooks.validOpsReplyAssistant(staleInitializingReplyAssistant,replyOverviewGeneratedMs,healthyThresholds),true,'a timestamp-null active transition still reconstructs worker_stalled from the last-run fallback age');
assert.equal(hooks.validOpsReplyAssistant({...staleInitializingReplyAssistant,status:'ok',reason_codes:[]},replyOverviewGeneratedMs,healthyThresholds),false,'a stale active transition cannot hide its reconstructed critical worker_stalled state');
assert.equal(hooks.validOpsReplyAssistant({...initializingReplyAssistant,status:'critical',reason_codes:['worker_stalled']},replyOverviewGeneratedMs,healthyThresholds),false,'a fresh run-start transition cannot synthesize a stale-worker critical reason');
...
assert.match(paymentOperationsHtml,/Stripe Connect provider capabilities: NOT COVERED[\s\S]*Connected-account balance: NOT COVERED[\s\S]*Bank-payout state: NOT COVERED[\s\S]*No current provider state may be inferred/,'Connect capabilities, balance, and bank-payout state remain explicit declared gaps with no inference');
...
for(const label of ['Expert subscriptions','AI credit checkouts','Group capacity payments','Group ticket checkouts','Written-reading payments','Payment receipt outbox']){
  assert.match(paymentOperationsHtml,new RegExp(label.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')),label+' is rendered from its authoritative aggregate');
}
const missingSubscriptionPaymentOperations={...healthyPaymentOperations};delete missingSubscriptionPaymentOperations.expert_subscriptions;
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:missingSubscriptionPaymentOperations}),false,'missing expert-subscription payment evidence fails the overview closed');
const missingPaymentActionRequiredStatus={...healthyPaymentOperations.expert_subscriptions.by_status};delete missingPaymentActionRequiredStatus.payment_action_required;
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:{...healthyPaymentOperations,expert_subscriptions:{...healthyPaymentOperations.expert_subscriptions,by_status:missingPaymentActionRequiredStatus}}}),false,'expert-subscription evidence fails closed when payment_action_required is omitted from the exact status vocabulary');
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:{...healthyPaymentOperations,ai_credit_checkouts:{...healthyPaymentOperations.ai_credit_checkouts,total:'0'}}}),false,'numeric-looking AI-credit checkout counts are rejected');
const missingAiQuarantineCount={...healthyPaymentOperations.ai_credit_checkouts};delete missingAiQuarantineCount.quarantined_count;
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:{...healthyPaymentOperations,ai_credit_checkouts:missingAiQuarantineCount}}),false,'AI-credit quarantine evidence is required by the exact contract');
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:{...healthyPaymentOperations,ai_credit_checkouts:{...healthyPaymentOperations.ai_credit_checkouts,quarantined_count:1}}}),false,'quarantined AI-credit rows cannot exceed failed rows');
const missingCapacityStatus={...healthyPaymentOperations.group_capacity.by_status};delete missingCapacityStatus.cancelled;
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:{...healthyPaymentOperations,group_capacity:{...healthyPaymentOperations.group_capacity,by_status:missingCapacityStatus}}}),false,'group-capacity payment evidence requires the exact status vocabulary');
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:{...healthyPaymentOperations,group_tickets:{...healthyPaymentOperations.group_tickets,open_count:1}}}),false,'group-ticket open counts must equal their exact open-status aggregate');
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:{...healthyPaymentOperations,on_demand:{...healthyPaymentOperations.on_demand,paid_undelivered_count:1}}}),false,'written-reading delivery counts cannot exceed paid authoritative rows');
const missingReceiptStatus={...healthyPaymentOperations.receipt_outbox.by_status};delete missingReceiptStatus.suppressed;
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:{...healthyPaymentOperations,receipt_outbox:{...healthyPaymentOperations.receipt_outbox,by_status:missingReceiptStatus}}}),false,'receipt-outbox evidence requires every exact delivery state');
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:{...healthyPaymentOperations,connect_provider_visibility:{...healthyPaymentOperations.connect_provider_visibility,inference_permitted:true}}}),false,'provider capabilities, balance, or bank-payout state can never be inferred from endpoint receipts');
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:{...healthyPaymentOperations,window:{...healthyPaymentOperations.window,checkout_stale_warning_after_sec:901}}}),false,'published paid-checkout thresholds must match the backend contract exactly');
const missingPaidCoverageSource={...healthyPaymentOperations.coverage.sources};delete missingPaidCoverageSource.ai_credit_checkouts_and_grants;
...
const processingRefundHtml=hooks.renderPaymentOperations(processingRefundOverview);
assert.match(processingRefundHtml,/0 awaiting admin[\s\S]*Stripe processing 1[\s\S]*Refund awaiting admin \/ Stripe processing \/ approved \/ declined \/ failed[\s\S]*0 \/ 1 \/ 0 \/ 0 \/ 0/,'Ops renders processing separately from untouched pending, terminal failures, and decisions');
assert.match(processingRefundHtml,/processing cannot be declined/,'Ops explicitly treats a Stripe-processing refund as in flight rather than admin-declinable');
assert.equal(hooks.validOpsOverview({...processingRefundOverview,summary:{...processingRefundOverview.summary,pending_refund_requests:0}}),false,'the summary in-flight count must include both untouched pending and Stripe-processing refunds');
...
const missingDisputeState={...healthyPaymentOperations.dispute_recovery.by_state};delete missingDisputeState.observed;
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:{...healthyPaymentOperations,dispute_recovery:{...healthyPaymentOperations.dispute_recovery,by_state:missingDisputeState}}}),false,'the durable dispute journal requires the complete exact recovery-state vocabulary');
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:{...healthyPaymentOperations,dispute_recovery:{...healthyPaymentOperations.dispute_recovery,active_count:'0'}}}),false,'numeric-looking dispute-recovery aggregates are rejected');
assert.equal(hooks.validOpsOverview({...healthyOverview,payment_operations:{...healthyPaymentOperations,dispute_recovery:{...healthyPaymentOperations.dispute_recovery,transfer_reversal_amount_cents:100,expert_clawback_cents:99}}}),false,'dispute expert clawback must exactly equal the transfer reversal minus returned application fee');
...
const newPaidDomainAlertSeverities={
  payment_operations_subscription_integrity:'critical',payment_operations_subscription_payment_attention:'warning',payment_operations_subscription_attempt_stale:'warning',
  payment_operations_ai_credit_integrity:'critical',payment_operations_ai_credit_checkout_stale_or_failed:'warning',
  payment_operations_group_capacity_integrity:'critical',payment_operations_group_capacity_checkout_stale_or_failed:'warning',
  payment_operations_group_ticket_integrity:'critical',payment_operations_group_ticket_checkout_stale_or_failed:'warning',
...
assert.equal(hooks.validOpsOverview({...healthyOverview,background_tasks:{...healthyOverview.background_tasks,suppressed:-1},summary:{...healthyOverview.summary,background_task_suppressed:-1}}),false,'negative suppression counters fail closed');
assert.equal(hooks.validOpsOverview({...healthyOverview,background_tasks:{...healthyOverview.background_tasks,suppressed:0.5},summary:{...healthyOverview.summary,background_task_suppressed:0.5}}),false,'fractional suppression counters fail closed');
const recipientSuppressionOverview={...healthyOverview,background_tasks:{...healthyOverview.background_tasks,suppressed:7},summary:{...healthyOverview.summary,background_task_suppressed:7}};
assert.equal(hooks.validOpsOverview(recipientSuppressionOverview),true,'intentional recipient-safety suppressions do not become dead-job or permanent-failure health');
const recipientSuppressionHtml=hooks.renderOperationalAuthorities(recipientSuppressionOverview);
...
assert.equal(unexpectedTargetFetches,0,'an admin bearer token is never sent when the configured Ops API base is not the exact expected production base');
...
assert.equal(actionIdentitySandbox.requireVerifiedMutationTarget(),false,'production Ops Monitor never authorizes a server mutation');
assert.match(actionIdentityFeedback[0],/production Ops Monitor is read-only/i);
...
actionRequestSandbox.OB_CLIENT_CONTEXT={
  capture:scope=>Object.freeze({scope,token:'admin-token',principal:'admin:1',role:'admin',signal:new AbortController().signal}),
  isCurrent:(_context,options)=>options&&options.exactCredential===true,
};
...
await assert.rejects(()=>actionRequestSandbox.testOpsApi('/admin/observability/cache/clear',{dry_run:true}),error=>error&&error.code==='ops_read_only','production refuses a mutating Ops request before fetch');
...
const reportSections=actionSandbox.safeNormalizedOpsReportSections({
  runtime_identity:{...verifiedIdentity,provider_environment_name:'production',node_environment:'production',classification_basis:'node_env_production',git_commit_sha:'abcdef1234567890',process_started_at:'2026-09-01T07:00:00Z',secret:'LEAK-ME'},
...
const paymentProvenanceReport=actionSandbox.safeNormalizedOpsReportSections({provenance:{payment_operations:{status:'collected',source:'all_paid_domain_database_aggregates_and_verified_webhook_receipt_ledger',authority:'runtime_authoritative_database',scope:'platform',window:'current_paid_domain_integrity_backlogs_trailing_24h_and_durable_verified_webhook_receipt_state',cache_semantics:'uncached_repeatable_read_database_snapshot_and_uncached_ledger_read',freshness_semantics:'database_and_verified_webhook_ledger_state_at_collection',age_basis:'collected_at',coverage:{complete:false,global_complete:false,database_complete:false,webhook_ledger_complete:true,end_to_end_delivery_complete:false,incomplete_sources:paidDomainSourceKeys.slice()},raw_account_ids:['acct_PRIVATE']}}});
assert.equal(paymentProvenanceReport.provenance.payment_operations.coverage.global_complete,false,'payment provenance preserves the new global completeness field');
assert.deepEqual(Array.from(paymentProvenanceReport.provenance.payment_operations.coverage.incomplete_sources),paidDomainSourceKeys,'payment provenance preserves only allowlisted aggregate source names');
assert.doesNotMatch(JSON.stringify(paymentProvenanceReport),/acct_PRIVATE|raw_account_ids/,'payment provenance cannot copy raw connected-account identifiers');

const fullSafeReport=actionSandbox.safeOpsReportSections({
  generated_at:'2026-09-01T08:00:00Z',
  summary:{snapshot_source:'backend_private_observability',primary_window:'live_trailing_15m_and_trailing_24h',runtime_environment:'production',runtime_classification:'production',runtime_deployment_id:'deploy-1',active_alerts:1,critical_alerts:0,warning_alerts:1,api_window_sec:900,api_observed_window_sec:900,api_window_coverage_complete:true,api_window_request_count:4,api_p95_ms:240,api_error_rate_pct:25,api_429_count:0,payment_operations_status:'ok',payment_operations_coverage_complete:true,payment_authorization_manual_review_jobs:0,payment_active_disputes:0,payment_dispute_manual_review:0,payment_dispute_lost_unrecovered:0,payment_dispute_compensation_pending:0,payment_outstanding_sessions:0,payment_outstanding_amount:0,pending_refund_requests:0,processing_refund_requests:0,failed_refund_requests:0,stripe_webhook_configuration_ready:true,security_level:'heavy',background_task_status:'ready',background_task_health:'degraded',background_task_health_code:'background_task_dropped_work',background_task_dead_classification_complete:true,background_task_dead_classification_fresh:true,background_task_dead_inspection_sampled_at:1756710200,background_task_suppressed:6,stripe_mode:'live',secret:'LEAK-ME'},
  ...
  payment_operations:{...payoutFailurePaymentOperations,expert_subscriptions:{...payoutFailurePaymentOperations.expert_subscriptions,customer_id:'cus_PRIVATE',stripe_subscription_id:'sub_PRIVATE',email:'alice@example.test'},ai_credit_checkouts:{...payoutFailurePaymentOperations.ai_credit_checkouts,checkout_session_id:'cs_PRIVATE',client_id:'private-client'},group_capacity:{...payoutFailurePaymentOperations.group_capacity,expert_id:'private-expert',payment_intent:'pi_PRIVATE'},group_tickets:{...payoutFailurePaymentOperations.group_tickets,registration_id:'private-registration',destination_account:'acct_PRIVATE'},on_demand:{...payoutFailurePaymentOperations.on_demand,request_id:'private-request',answer:'LEAK-ME'},receipt_outbox:{...payoutFailurePaymentOperations.receipt_outbox,recipient:'alice@example.test',message_id:'private-message'},connect_provider_visibility:{...payoutFailurePaymentOperations.connect_provider_visibility,stripe_account_id:'acct_PRIVATE'},webhook:{...payoutFailureWebhook,provider_secret:'whsec_LEAK-ME',recent_financial_events_24h:{...payoutFailureWebhook.recent_financial_events_24h,connected_account:'acct_secret'}},private_session_ids:['secret-session'],secret:'LEAK-ME'},
...
assert.equal(fullSafeReport.payment_operations.coverage.global_complete,true,'the copied payment report preserves global payment-evidence completeness separately from provider-side gaps');
assert.equal(fullSafeReport.payment_operations.coverage.sources.expert_subscriptions_and_attempts,'complete');
...
assert.equal(fullSafeReport.payment_operations.group_capacity.total,0);
assert.equal(fullSafeReport.payment_operations.group_tickets.total,0);
assert.equal(fullSafeReport.payment_operations.on_demand.total,0);
assert.equal(fullSafeReport.payment_operations.receipt_outbox.total,0);
assert.equal(fullSafeReport.payment_operations.connect_provider_visibility.capability_state,'not_covered');
assert.equal(fullSafeReport.payment_operations.connect_provider_visibility.balance_state,'not_covered');
assert.equal(fullSafeReport.payment_operations.connect_provider_visibility.bank_payout_state,'not_covered');
...
assert.equal(fullSafeReport.summary.processing_refund_requests,0,'the bounded summary projection preserves the processing-only refund count');
...
assert.equal(fullSafeReport.summary.background_task_suppressed,6,'the bounded summary report preserves the recipient-safety suppression count');
assert.equal(fullSafeReport.background_tasks.suppressed,6,'the background-task report preserves suppressions separately from dead jobs');
assert.match(fullSafeReport.background_tasks.suppression_semantics,/not dead or permanently failed jobs/i,'the copied report labels suppression semantics explicitly');
...
assert.equal(fullSafeReport.payment_runtime.configured_mode,'live','the report preserves active Stripe configuration readiness and coherent configured mode');
...
const webhookReadinessProjection=actionSandbox.safeOpsReportSections({
  ...
  payment_runtime:{available:true,status:'warning',active_stripe_mode:'live',active_configuration_ready:true,platform_webhook_configuration_ready:true,connect_webhook_configuration_ready:false,webhook_configuration_ready:false,configured_mode:'live',last_error_code:'stripe_connect_webhook_configuration_unavailable',signing_secret:'LEAK-ME'},
...
assert.equal(paidDomainAlertReport.alerts.find(alert=>alert.key==='payment_operations_subscription_payment_attention').meta.by_status.payment_action_required,1,'payment_action_required attention evidence survives the bounded developer-report projection');
assert.doesNotMatch(JSON.stringify(paidDomainAlertReport),/alice@example|acct_PRIVATE|private\/payment/,'new paid-domain alerts cannot copy PII, provider IDs, or internal paths');
assert(paidDomainAlertReport.alerts.every(alert=>!Object.hasOwn(alert.meta,'recipient')&&!Object.hasOwn(alert.meta,'stripe_account_id')),'unknown identity-bearing alert metadata fields are omitted');
...
assert.match(actionSource,/function requireVerifiedMutationTarget\(\)[\s\S]*OPS_SERVER_MUTATIONS_ENABLED===true[\s\S]*ops_read_only[\s\S]*return false/,'production mutation authorization is unconditionally fail-closed');
...
const stopControlSandbox={window:null,Error,Object,Array,String,Number,JSON,Promise,
  ...
  opsGet:async()=>({}),opsEnvironment:()=> 'production',opsIdentityVerification:()=>({verified:true}),copyText:async()=>{},location:{origin:'https://ownlybiz.com',pathname:'/admin',href:'https://ownlybiz.com/admin'},formatOpsEmergencyFailure:error=>String(error.message),setTimeout(){},setOpsActionButtonsBusy(){},
};
...
async function runFingerprintControl(action,previewEnvelope,{prompts=[]}={}){
  ...
  const control={window:null,Error,Object,Array,String,Number,JSON,Promise,
    ...
    actionResult:(...args)=>feedback.push(args),applyOpsOutcome:(...args)=>feedback.push(args),refreshAfterAction(){},opsGet:async()=>({}),opsEnvironment:()=> 'production',opsIdentityVerification:()=>({verified:true}),copyText:async()=>{},location:{origin:'https://ownlybiz.com'},formatOpsEmergencyFailure:error=>String(error.message),setTimeout(){},setOpsActionButtonsBusy(){},
  };
...
const mismatchControl={window:null,Error,Object,Array,String,Number,JSON,Promise,
  ...
  opsContractError:message=>new Error(message),actionResult:(...args)=>mismatchFeedback.push(args),applyOpsOutcome:(...args)=>mismatchFeedback.push(args),refreshAfterAction(){},obRefreshOpsDashboard:async()=>({ok:true}),opsGet:async()=>({}),opsEnvironment:()=> 'production',opsIdentityVerification:()=>({verified:true}),copyText:async()=>{},location:{origin:'https://ownlybiz.com'},formatOpsEmergencyFailure:error=>String(error.message),setTimeout(){},setOpsActionButtonsBusy(){},
};
...
assert.match(mismatchFeedback[0][0],/candidate fingerprint[\s\S]*changed[\s\S]*No mutation was applied/i);
...
assert.match(opsSource,/data-ob-focus-key="action:refresh"[\s\S]*data-ob-focus-key="action:copy-report"/,'the two read-only production controls expose stable semantic focus keys');
assert.doesNotMatch(hooks.renderSafeActionsTab(),/clear_cache|cleanup_stale|cleanup_pending|stop_one|stop_all|Clear server cache|Emergency stop/,
  'the production Safe actions tab omits all server-mutating controls');
...
const projectedProductionGuide=alertClipboardSandbox.__OB_TEST_HOOKS__.opsOwnerGuide.projectGuide(injectedBackendGuideAlert);
...
assert.doesNotMatch(JSON.stringify(projectedProductionGuide),/cleanup_stale|stop_all|clear_cache|Settle now|Stop all|Clear cache|RUN PROD MUTATION|INJECTED IMPACT/,'backend guide prose and mutating actions cannot enter the production owner projection');
...
for(const [key,severity] of Object.entries(newPaidDomainAlertSeverities)){
  const projected=alertClipboardSandbox.__OB_TEST_HOOKS__.opsAlertClipboard.projectedAlertHandoff({key,severity,title:'alice@example.test',detail:'acct_PRIVATE /private/payment.json',at:'2026-09-01T08:00:00Z',meta:{integrity_mismatch_count:1,stale_open_count:1,recipient:'alice@example.test',stripe_account_id:'acct_PRIVATE',by_status:{...healthyPaymentOperations.expert_subscriptions.by_status,payment_action_required:1}}});
  assert.equal(projected.key,key,key+' is accepted by the owner alert-code allowlist');
  assert.notEqual(projected.title,'Unrecognized alert; free-form prose omitted.',key+' has a stable allowlisted owner title');
  assert(projected.impact&&projected.owner_action,key+' has bounded owner guidance');
  assert.doesNotMatch(JSON.stringify(projected),/alice@example|acct_PRIVATE|private\/payment/,key+' omits PII, provider IDs, and internal paths');
  assert.equal(Object.hasOwn(projected.meta,'stripe_account_id'),false,key+' omits unknown provider-ID fields');
}
const paymentActionRequiredProjection=alertClipboardSandbox.__OB_TEST_HOOKS__.opsAlertClipboard.projectedAlertHandoff({key:'payment_operations_subscription_payment_attention',severity:'warning',meta:{payment_attention_count:1,by_status:{...healthyPaymentOperations.expert_subscriptions.by_status,payment_action_required:1}}});
...
assert.doesNotMatch(html,/processPayout\s*:|api\('PUT',`\/admin\/payouts/,
  'no public frontend helper can mutate a legacy internal payout record as if it were a Stripe bank payout');
assert.match(html,/id="stripe-webhook-secret"[\s\S]*id="stripe-connect-webhook-secret"/,
  'Admin Platform Payments exposes separate platform and Connect signing-secret inputs');
assert.match(html,/existingSetting\(settings, mode, 'connect_webhook_secret'\)/,
  'mode switching loads the masked Connect signing-secret presence independently');
assert.match(html,/body\['stripe_' \+ mode \+ '_connect_webhook_secret'\] = connectWh/,
  'saving Stripe configuration sends the Connect signing secret to the selected test or live field');
assert.match(html,/Platform and Connect webhook signing secrets must be different/,
...
assert.match(html,/Production payment model: client payments are destination charges through Stripe Connect/,
  'production Platform Payments identifies the shared Stripe Connect payment model');
assert.doesNotMatch(html,/Payment model for this environment:/,
  'the production candidate does not retain staging-generic payment copy');
...
assert.match(html,/<div class="db-stripe-status" data-ob-stripe-state="loading"[^>]*>Checking Stripe…<\/div>/,
  'the expert sidebar also starts neutral instead of claiming a connection before the readiness fetch');
assert.doesNotMatch(html,/>\s*● Stripe Connected\s*</,
  'no first-paint expert payment surface claims Stripe is connected');
assert.match(html,/setExpertStripeAction\('unknown',null\)[\s\S]*No connection, capability, balance, or bank-payout state is inferred/,
...
assert.match(html,/Connected-account receipts accepted here can be authenticated[\s\S]*Current capabilities, balance, and bank-payout state still require Stripe reconciliation/,
...
assert.doesNotMatch(html,/Default Payout Schedule|Minimum Payout Threshold|Payout hold period|Auto-approve payouts/,
  'Ownlybiz does not expose legacy controls as Stripe Connect bank-payout controls');
assert.match(html,/onclick="adminNav\(this,'payouts'\)"[^>]*>[\s\S]*?Connect Records<\/div>/,
  'the static admin navigation names the historical view without a fake pending payout count');
assert.doesNotMatch(html,/>8 pending</,
  'no first-paint admin navigation claims a fabricated payout queue count');
assert.match(html,/OPS_PAYMENT_REFUND_STATUSES=Object\.freeze\(\['pending','processing','approved','declined','failed'\]\)/,
  'the strict Ops refund contract includes Stripe processing as a distinct state');
assert.match(html,/Processing with Stripe[\s\S]*Approval and decline are disabled/,
  'admin refund surfaces identify Stripe processing as in flight and non-declinable');
assert.match(html,/decisionStatus === 'processing'[\s\S]*Stripe accepted the refund attempt\. It is processing; wait for the signed terminal outcome/,
  'admin approval feedback reports Stripe acceptance as processing rather than a completed refund');
assert.doesNotMatch(html,/Refund approved and sent to Stripe\.|then\(function\(\)\{[^}]*Refund approved\./,
  'approval success handlers do not announce a terminal refund before the reconciled status is known');
assert.match(html,/status === 'processing'\) return '<span class="ob-session-refund-pill processing">Refund processing with Stripe<\/span>'/,
  'expert-facing refund state blocks duplicate actions while Stripe is processing');
const onDemandDeclineSource=section(html,'window.obOdDecline = function(id){','  function obOdAutopilotErrorMessage');
assert.match(onDemandDeclineSource,/queued_for_admin_approval===true&&result\.refund_request[\s\S]*full refund is queued for admin review; payment remains paid until Stripe confirms the outcome/,
  'paid written-reading decline reports the new admin-review queue without claiming a refund');
assert.match(onDemandDeclineSource,/provider_refund_confirmed===true[\s\S]*Stripe-confirmed refund recorded/,
  'the UI reserves confirmed-refund wording for an explicit provider-confirmation signal');
assert.doesNotMatch(onDemandDeclineSource,/Request declined\/refunded|Request declined and refunded/,
  'written-reading decline never conflates a declined service request with a provider-confirmed refund');
assert.match(html,/Decline & request refund review/,
  'the expert action is labeled as a review request rather than an immediate refund');
