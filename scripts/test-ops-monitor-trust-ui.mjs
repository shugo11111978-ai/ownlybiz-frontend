...
const authorities=hooks.renderOperationalAuthorities({
  ...
  payment_runtime:{available:true,status:'ok',active_stripe_mode:'live',active_configuration_ready:true,configured_mode:'live',last_error_code:null},
...
const healthyOverview=withOverviewProvenance({
  ...
  business:healthyBusiness,realtime:backendRealtime,
  ...
  media_sfu:healthyMediaSfu,security:healthySecurity,reply_assistant:healthyReplyAssistant,payment_runtime:{available:true,status:'ok',active_stripe_mode:'test',active_configuration_ready:true,configured_mode:'test',last_error_code:null},
});
...
assert.equal(hooks.validOpsOverview({...healthyOverview,provenance:{}}),false,'a trusted envelope requires all 16 exact section provenance records');
...
new vm.Script(helperSource+'\nthis.requirePreview=requirePreview;this.requireCandidateFingerprint=requireCandidateFingerprint;this.isCandidateFingerprintMismatch=isCandidateFingerprintMismatch;this.requiredCount=requiredCount;this.classifyOpsResponse=classifyOpsResponse;this.applyOpsOutcome=applyOpsOutcome;this.safeNormalizedOpsReportSections=safeNormalizedOpsReportSections;this.safeOpsReportSections=safeOpsReportSections;').runInContext(actionSandbox);
...
const fullSafeReport=actionSandbox.safeOpsReportSections({
  ...
  payment_runtime:{available:true,status:'ok',active_stripe_mode:'test',active_configuration_ready:true,configured_mode:'test',stripe_key:'LEAK-ME'},
...
assert.doesNotMatch(actionSource,/summary:overview\.summary|alerts:overview\.alerts|http:overview\.http|business:overview\.business|realtime:overview\.realtime|assets:overview\.assets|cache:overview\.cache|storage:overview\.storage|background_tasks:overview\.background_tasks|live_capacity:overview\.live_capacity|payment_runtime:overview\.payment_runtime|media_sfu:overview\.media_sfu|security:overview\.security|reply_assistant:overview\.reply_assistant|system:overview\.system|trends:trends|events:events/,'no raw overview, trend, or event section bypasses the clipboard projectors');
