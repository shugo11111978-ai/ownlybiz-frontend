...
ctaContext.planState.publicOfferFailed=true;vm.runInNewContext('updateSignupCta();',ctaContext);assert.equal(account.disabled,true);assert.equal(cta.disabled,true);
assert.match(presentation.render(offer,'pricing','monthly',null,true),/Try again/);assert.doesNotMatch(presentation.render(offer,'pricing','monthly',null,true),/\$|href="\/signup/);
ctaContext.planState.publicOfferFailed=false;vm.runInNewContext('updateSignupCta();',ctaContext);assert.equal(account.disabled,false);assert.equal(cta.disabled,false);
