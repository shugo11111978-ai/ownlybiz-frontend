...
const native=source('  function setSignupPlan(plan, interval){','  function currentExpertSlug(){')+source('  async function finishSignupLaunch(){','  function signupCheckoutPlanName(')+source('  function signupCheckoutPlanName(','  function renderStarterPendingApproval(')+source('  function applyMarketingRoute(route, event){','  function applyGroupRoute(');
...
function harness({role='expert',authenticated=true,search='?resume=1',approval=approved,billingPatch={},authPatch={}}={}){
 const qa={credential:authenticated?'expert-token':'',user:authenticated?{id:'owner',role}: {},identity:1,step:1,requests:[],redirects:[],checkouts:[],renders:[],offerRoutes:0,notices:[],history:[],response:null};
 ...
 const insertNotice=card=>{nodes[card.id]=card;qa.notices.push(card.innerHTML);};
 const launch={firstChild:null,insertBefore:insertNotice};
 nodes['step-7'].querySelector=selector=>selector==='.launch-screen'?launch:null;
 nodes['step-7'].insertBefore=insertNotice;
 ...
 const location={pathname:'/signup',search,replace:p=>qa.redirects.push({method:'replace',path:p}),assign:p=>qa.redirects.push({method:'assign',path:p})};
 Object.defineProperty(location,'href',{get:()=>`https://ownlybiz.com${location.pathname}${location.search}`});
 const ctx={console,AbortController,URLSearchParams,URL,setTimeout,clearTimeout,qa,planState:{selected:'starter',interval:'monthly',plans,publicOffer:offer},planDefaults:plans,location,history:{state:null,replaceState(_state,_title,path){const url=new URL(path,'https://ownlybiz.com');location.pathname=url.pathname;location.search=url.search;qa.history.push(url.pathname+url.search);}},sessionStorage:{getItem:k=>stored.get(k)||null,setItem:(k,v)=>stored.set(k,v)},document:{getElementById:id=>nodes[id]||null,createElement:()=>({style:{},remove(){if(this.id)delete nodes[this.id];}}),querySelector:s=>s==='#view-2.active'?{}:null,querySelectorAll:()=>[]},token:()=>qa.credential,readUserSnapshot:()=>qa.user,currentBillingRole:()=>qa.user.role||'',normalizePlanId:p=>p,usesSubscriptionOffer:()=>true,selectedPlan:()=>plans.find(p=>p.id===ctx.planState.selected),selectedSignupPlanForLaunch:()=>plans.find(p=>p.id===ctx.planState.selected),plansById:ps=>Object.fromEntries(ps.map(p=>[p.id,p])),esc:s=>String(s??''),signupPayoutEligibility:()=>ctx.planState.billing?.payout_country_eligibility,publicOfferRenderer:()=>({...presentation,updateSignup:(_target,_data,kind,interval,plan)=>qa.renders.push({kind,interval,plan})}),goStep:n=>qa.step=n,switchView:()=>{},applyPlatformFallbackSeo:()=>{},obApplySignupOfferRoute:()=>{qa.offerRoutes++;},obJson:async(path,options)=>{qa.requests.push({path,method:options.method});assert.equal(options.method,'GET');if(qa.response)return qa.response(path,options);return path==='/api/auth/me'?auth:billing;},openSubscriptionCheckout:async(...args)=>qa.checkouts.push(args)};
...
}
{
 ...
 assert.equal(restoring.qa.notices.length,1);
 assert.match(restoring.qa.notices[0],/Stripe checkout was not completed/);
 assert.match(restoring.qa.notices[0],/subscription is not active/);
 assert.deepEqual(restoring.qa.history,['/signup?resume=1']);
 assert.equal(restoring.ctx.location.search,'?resume=1');
 ...
 restoring.ctx.applyMarketingRoute({page:'signup'});
 await restoring.settle();
 assert.equal(restoring.qa.notices.length,1,'the cancelled Checkout notice is rendered once by the resume owner');
 assert.equal(restoring.qa.offerRoutes,0,'the canonical resume URL stays out of the fresh-signup marketing path');
}
...
for(const role of ['admin','client']){
 const h=harness({role});await h.resume();assert.equal(h.qa.requests.length,0);assert.equal(h.qa.step,1);assert.equal(h.qa.redirects[0].path,'/login?resume=signup');assert.equal(h.stored.get('ob_next'),'/signup?resume=1');
 const bare=harness({role,search:''});assert.equal(bare.ctx.obResumeSignupRoute(),false);bare.qa.step=7;bare.ctx.applyMarketingRoute({page:'signup'},{obExplicitNavigation:true});assert.equal(bare.qa.step,1);assert.equal(bare.qa.offerRoutes,1);assert.equal(bare.qa.redirects.length,0);assert.equal(bare.qa.requests.length,0);
 const mismatch=harness({authPatch:{user:{id:'owner',role}}});await mismatch.resume();assert.equal(mismatch.qa.requests.length,1);assert.equal(mismatch.qa.redirects[0].path,'/login?resume=signup');assert.equal(mismatch.stored.get('ob_next'),'/signup?resume=1');
}
...
// Execute the original login handler through its successful login and redirect.
{
 ...
 const ctx={console:{log(){},error(){}},URL,URLSearchParams,location:{origin:'https://ownlybiz.com',search:'?resume=signup'},document:{getElementById:id=>nodes[id]||null},sessionStorage:{getItem:k=>stored.get(k)||'',setItem:(k,v)=>stored.set(k,v),removeItem:k=>stored.delete(k)},localStorage:{setItem(){},removeItem(){}},setTimeout:fn=>deferred.push(fn),obBeginAuthAttempt:()=>({}),obAuthAttemptCurrent:()=>true,obCommitAuthAttempt:()=>({}),fetch:async path=>({ok:true,status:200,json:async()=>path.endsWith('/login')?{token:'signed-in',user}: {}})};ctx.window=ctx;vm.createContext(ctx);vm.runInContext(source('    window.obLoginReturnPath = function(value, role) {','  </script>'),ctx);await ctx.handleMarketingLogin();deferred.forEach(fn=>fn());assert.equal(ctx.location.href,'/signup?resume=1');assert.equal(stored.has('ob_next'),false);
}
{
 const stored=new Map([['ob_next','/signup?resume=1&billing=cancelled&checkout_plan=pro']]);let committed=0;
 const nodes={'mkt-login-email':{value:'admin@example.invalid'},'mkt-login-pass':{value:'password'},'mkt-login-btn':{textContent:'Sign in'},'mkt-login-error':{style:{},textContent:''}};
 const ctx={console:{log(){},error(){}},URL,URLSearchParams,location:{origin:'https://ownlybiz.com',search:'?resume=signup'},document:{getElementById:id=>nodes[id]||null},sessionStorage:{getItem:k=>stored.get(k)||'',setItem:(k,v)=>stored.set(k,v),removeItem:k=>stored.delete(k)},localStorage:{setItem(){},removeItem(){}},setTimeout(){},obBeginAuthAttempt:()=>({}),obAuthAttemptCurrent:()=>true,obCommitAuthAttempt:()=>{committed++;return{};},fetch:async()=>({ok:true,status:200,json:async()=>({token:'admin-token',user:{id:'admin',role:'admin',name:'Admin'}})})};ctx.window=ctx;vm.createContext(ctx);vm.runInContext(source('    window.obLoginReturnPath = function(value, role) {','  </script>'),ctx);await ctx.handleMarketingLogin();assert.equal(committed,0);assert.match(nodes['mkt-login-error'].textContent,/expert account/);assert.equal(nodes['mkt-login-error'].style.display,'block');assert.equal(stored.get('ob_next'),'/signup?resume=1&billing=cancelled&checkout_plan=pro');assert.equal(ctx.location.href,undefined);
}
